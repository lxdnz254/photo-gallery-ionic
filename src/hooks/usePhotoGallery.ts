...
export function usePhotoGallery() {
    ...
    const takePhoto = async () => {
        ...
        set(PHOTO_STORAGE,
            ...
            // Dont't save the base64 representation of the photo data,
            // since its already saved on the Filesytem.
            const photoCopy = { ...p };
    ...
    const getPhotoFile = async (cameraPhoto: CameraPhoto, fileName: string): Promise<Photo> => {
        if (isPlatform('hybrid')) {
            // Get the new, complete filepath of the photo saved on the filesystem
